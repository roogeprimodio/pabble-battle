...
import {
  ...
  canRemovePawn,
  ADJACENCY_LIST,
  GamePhase as LocalGamePhase, // Renamed to avoid conflict
...
const NinePebblesPage: React.FC = () => {
  ...
  const [localGamePhase, setLocalGamePhase] = useState<LocalGamePhase>('placement'); // For actual game mechanics
  ...
  useEffect(() => {
    ...
    if (existingRoomId) {
      ...
      setLocalPlayer(2); // Assume joiner is Player 2
      setCurrentPlayer(1); // Game always starts with Player 1
      setOverallPagePhase('playing');
      setLocalGamePhase('placement');
      // Initialize pawn placement for Player 1 (Angels) who starts the online game
  ...
  const updateMessageAndPawnsToPlace = useCallback(() => {
    ...
    if (overallPagePhase !== 'playing') {
        // Messages for setup phases are handled by those screens
  ...
  const resetGameState = () => {
    ...
    setLocalGamePhase('placement'); // Default start for game logic
    setCurrentPlayer(1);
    setPlayerStats({
      1: { pawnsToPlace: PAWNS_PER_PLAYER, pawnsOnBoard: 0, hasCompletedInitialTwoPawnPlacement: false },
      2: { pawnsToPlace: PAWNS_PER_PLAYER, pawnsOnBoard: 0, hasCompletedInitialTwoPawnPlacement: false },
  ...
  const handleLocalPlayerSelect = (player: Player) => {
    setIsLoading(true);
    resetGameState(); // Reset game specific states
    setCurrentPlayer(player);
    setLocalGamePhase('placement');
    setOverallPagePhase('playing'); // Now playing locally
    setLocalPlayer(null); // No specific local player in local mode
    setRoomId(null);
  ...
  const handleCreateRoomOnline = () => {
    ...
    setRoomId(newRoomId);
    setLocalPlayer(1);
    setCurrentPlayer(1); // Angels always start
    setOverallPagePhase('onlineWaitingForOpponent');
    setLocalGamePhase('placement');
    setPawnsToPlaceThisTurn(Math.min(2, PAWNS_PER_PLAYER)); // Angels place 2
    setIsCurrentPlayerOnInitialTwoPawnTurn(true);
  ...
  const handleJoinRoomOnline = () => {
    ...
    setLocalGamePhase('placement');
    setPawnsToPlaceThisTurn(Math.min(2, PAWNS_PER_PLAYER)); // Angels place 2
    setIsCurrentPlayerOnInitialTwoPawnTurn(true); // For the starting player (Angels)
  ...
  const renderOnlineWaitingScreen = () => (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-background to-secondary/20 p-2 sm:p-4 items-center justify-center">
      <header className="absolute top-4 left-4 right-4 flex justify-between items-center">
        <Button variant="outline" size="icon" aria-label="Back" onClick={() => {handleFullReset(); /* Go back to initial setup */}}><ArrowLeft className="h-5 w-5" /></Button>
        <ThemeToggle />
      </header>
      <Card className="w-full max-w-md shadow-2xl text-center animate-fadeIn">
        <CardHeader className="p-4 sm:p-6">
  ...
  const renderGameScreenSkeleton = () => (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-background to-secondary/20 p-2 sm:p-4">
      ...
      <footer className="text-center py-4 mt-auto text-sm text-muted-foreground">&copy; {new Date().getFullYear()} Pebble Arena</footer>
    </div>
  );
      ...
      <main className="flex-grow w-full flex flex-col items-center justify-center">
        <div className="w-full max-w-[calc(100vw-20px)] sm:max-w-xs md:max-w-md lg:max-w-lg xl:max-w-xl aspect-square relative self-center">
          <GameBoardDisplay
            ...
            disabled={roomId !== null && localPlayer !== currentPlayer && overallPagePhase === 'playing'}
          />
        </div>
      </main>
