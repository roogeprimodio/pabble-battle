...
import { Gem, Shield, Axe } from 'lucide-react'; // Example icons
        ...
        row.map((cell, cIndex) => {
          ...
          return (
            <button
              ...
              disabled={disabled}
              aria-label={`Cell ${rIndex},${cIndex}. Terrain: ${cell.terrain}. ${unit ? `Unit: ${unit.type}` : ''} ${shard ? `Shard present` : ''}`}
              className={`
                ...
                ${isSelectedUnit ? (unit?.player === 1 ? 'ring-2 ring-primary' : 'ring-2 ring-destructive') : ''}
            ...
            >
              ...
              {unit && (
                <div className={`transform scale-[0.35] sm:scale-[0.45] z-10 ${isSelectedUnit ? 'animate-subtle-glow' : ''}`}>
